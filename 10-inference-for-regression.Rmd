...
index <- which(Geyser_data$Duration == 211 & Geyser_data$Waiting == 178)
target_point <- mod_eruption |>
  get_regression_points() |>
  slice(index)
...
# Plot residual
best_fit_plot <- ggplot(Geyser_data, aes(x = Duration, y = Waiting)) +
  geom_point() +
  labs(
...
We can obtain all the $n = `r n`$ residuals by applying the `get_regression_points()` function to the regression model `mod_eruption`. Observe how the resulting values of `residual` are roughly equal to `Waiting - Waiting_hat` (there may be a slight difference due to rounding error).

```{r}
# Fit regression model:
simple_model <- lm(Waiting ~ Duration, data = Geyser_data)
...
One of the most useful plots is called the *residual plot*. We obtain


##### Linearity of relationship

We want to check whether the association between the response $y_i$ and the explanatory variable $x_i$ is **L**inear. Recall the scatterplot in Figure \@ref(fig:regline) where we had the explanatory variable $x$ as life expectancy and the outcome variable $y$ as fertility rate. Would you say that the relationship between $x$ and $y$ is linear? It's hard to say because of the scatter of the points about the line. In the authors' opinions, we feel this relationship is "linear enough."
...
For our UT Austin data, while there is data on `r n_Geyser_data` courses, these `r n_Geyser_data` courses were actually taught by `r evals |> select(prof_ID) |> n_distinct()` unique instructors. In other words, the same professor is often included more than once in our data. The original `evals` data frame that we used to construct the `Geyser_data` data frame has a variable `prof_ID`, which is an anonymized identification variable for the professor:

```{r}
evals |> 
  select(ID, prof_ID, Waiting, Duration_avg)
```
...
normal_and_not_examples <- Geyser_data |>
  mutate(
  ...
  select(Duration_avg, `Clearly normal`, `Clearly not normal`) |>
  gather(type, eps, -Duration_avg) |>
  ggplot(aes(x = eps)) +
  geom_histogram(binwidth = 0.25, color = "white") +
  labs(x = "Residual") +
...
```{r, eval=FALSE}
ggplot(regression_points, aes(x = Duration_avg, y = residual)) +
  geom_point() +
...
Geyser_data |>
  mutate(eps = (rnorm(n(), 0, 0.075 * Duration_avg^2)) * 0.4) |>
  ggplot(aes(x = Duration_avg, y = eps)) +
  geom_point() +
  labs(x = "Beauty Score", y = "Residual") +
  geom_hline(yintercept = 0, col = "blue", size = 1)
```
...
First, the **I**ndependence condition. The fact that there exist dependencies between different rows in `Geyser_data` must be addressed. In more advanced statistics courses, you'll learn how to incorporate such dependencies into your regression models. One such technique is called *hierarchical/multilevel modeling*.
...
- Use the `get_regression_points()` function to get the observed values, fitted values, and residuals for all `r n_Geyser_data` instructors. 
