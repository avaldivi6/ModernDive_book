...
We introduce a mathematical model to represent

We say that there is a linear relationship between $Y$ and $X$ if the response variable, $Y$, can be expressed as 
\begin{equation}
\label{mean}
Y = \beta_0 + \beta_1 X  + \epsilon
\end{equation} 

where $\beta_0$ (intercept) and $\beta_1$ (slope) are the parameters of the model, $X$ is the value of the explanatory variable and $\epsilon$ is called the random error and it is a random variable that represents a variation of the response $Y$ not explained by this model. Since it is a random variable, we assume that it has expected value 0 and variance equal to $\sigma^2$ which is another parameter of the model.
...
### Estimating the regression coefficients from data
...
### Hypothesis test for estimators
