...
Recall that each row in `UN_data_ch6` represents a particular country or UN member state. The `summary()` function shows a summary for each variable in the data set and we perform this analysis using the numerical variables life expectancy, `life_exp`, fertility rate, `fert_rate`, and the categorical variable income group, `income`. When the variable is numerical, the `summary()` function provides the five-number summary and also shows the mean. As an illustration, observe that the average life expectancy is `r UN_data_ch6$life_exp |> mean() |> round(digits = 2)` years. The first quartile, `1st Qu.`, shows that 25% of the UN member states have life expectancy less than 69.36 years. Similarly, there are four categories or factor levels for `income`: `Low Income`, `Lower middle income`, `Upper middle income`, and `High income`. The summary also provides the number of states for each factor level; for example, there are `r UN_data_ch6 |> filter(income == "High income") |> nrow()` `High Income` states.
