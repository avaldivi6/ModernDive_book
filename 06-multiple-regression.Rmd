...
Recall that each row in `UN_data` represents a particular country or UN member state. The `summary()` function shows a summary for each variable in the data set and we perform this analysis using the numerical variables life expectancy, `life_exp`, fertility rate, `fert_rate`, and the categorical variable income group, `income`. When the variable is numerical, the `summary()` function provides the minimum value observed, `Min.`, the first quartile, `1st Qu.`, second quartile or median, `Median`, third quartile, `3rd Qu.`, and maximum value observed, `Max.`. These five numbers together are often called the five-number summary. In addition, the `summary()` function provides the average or mean, `Mean`. We have encounter the mean before, but we briefly discussed what the quartiles now. The first quartile, `1st Qu.`, indicates that one fourth of the data is less than or equal to this value, the median or second quartile, `Median`, show that one half (two quarts) of the observations are less than or equal to this valueIf some numerical summary if the varaibelWe have no missing data, that there are `r UN_data |> filter(income == "Low income") |> nrow()` UN member states. The variable `income` is categorical with four categories; `Low Income`, `Lower middle income`, `Upper middle income`, and `High income` states. The summary also provides the number of states for each category; for example, there are `r UN_data |> filter(income == "High income") |> nrow()` `High Income` states. The average life expectancy is `r UN_data$life_exp |> mean() |> round(digits = 2)` years. The first quartile, `1st Qu.`, state that one quarter of the UN member states, or 25%, have life expectancy less than 69.36 years. The first quartile is also called the 25th percentile
...
$$\widehat{\text{fert rate}} = b_0 + b_1\cdot x = `r b_int[[1]]` + `r b_int[[5]]`\cdotx$$
...
Note that income is in thousands of dollars while debt and credit limit are in dollars. We can also compute summary statistics using the `summary()` function. We only `select()` the columns of interest for our model:

```{r, eval = FALSE}
credit_ch6 |> select(debt, credit_limit, income) |> summary()
```
...
The mean and median credit card `debt` are \$520.01 and \$459.50, respectively. The first quartile for `debt` is 68.75; this means that 25% of card holders had debts of \$68.75 or less. Correspondingly, the mean and median credit card limit, `credit_limit`, are \$4,736 and \$4,622, respectively. Note also that the third quartile of `income` is 57.47; this means that 75% of card holders had incomes below \$57,470.
...
<!--
I've added the correlation property of invariance under linear transformations in the main text. The Simpson's Paradox example presenting by separating the data in 4 groups based on credit limit should be moved to an appendix.
-->
