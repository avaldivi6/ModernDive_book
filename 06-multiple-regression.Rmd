...
coef(model_no_int)
```
```{r regtable-parallel, echo=FALSE, purl=FALSE}
# Fit regression model:
...
```{r, echo = FALSE}
credit_ch6 |> select(debt, credit_limit, income) |> summary() |> kable()
```
...
```{r, echo=FALSE}
credit_ch6 |> get_correlation(debt ~ 1000*income)|> kable()
```
