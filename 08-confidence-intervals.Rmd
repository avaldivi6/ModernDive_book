...
We studied sampling in Chapter \@ref(sampling). Recall, for example, getting many random samples of red and white balls consisted from a bowl, obtaining the sample proportions of red balls from each of those samples, and studying the distribution of the sample proportions. In summary, we learned that
...
Similarly, when sampling chocolate covered almonds and getting the sample mean weight from each sample, the characteristics described above are also encountered on the sampling distribution of the sample mean; namely,
...
Moreover, these characteristics also apply to sampling distributions for the difference of samples means, the difference of sample proportions, and many other parameters. Note that the sampling distribution is not dependent on the original distribution of the population values. As long as the samples taken are fairly large and we use the appropriate standard error, see \@ref(table-title), we can generalize these results appropriately.



The study of the sampling distribution is actually motivated by another question we have not yet answered: how can we determine the average weight of all the almonds from if we do not have access to the entire bowl?

Clearly, if we have access to a large number of random samples from the bowl, because of our study of the sampling distribution, we know that the average of the sample means will be the expected value of the sample mean, which is precisely the population mean weight.

However, in real life situations, we do not have access to many random samples. 
For most problems we have only access to a single random sample due to time or other constraints. 
Due to sampling variation we know that the sample mean will not match the population mean exactly, rather it will be an approximation of the population mean. We call this a __point estimate__ of the population mean.  Moreover, the information we get from the sample can also be used to obtain and interval of plausible values that __estimate__ the population mean. We call this interval an _interval estimate_, or if given some level of accuracy, a __confidence interval__  of the population mean. 
This chapter explain how to do this, presents different methods to produce confidence intervals, and shows why this is a clever idea that produces useful results.

In Section \@ref(traditional_CI) we introduce a method that uses information of the sample observed and characteristics of the sampling distribution of the sample mean to construct these intervals. We call this the traditional method to construct intervals. In addition, we present syntax in R to construct these confidence intervals and discuss how to interpret them. In Section \@ref(bootstrap_CI) we introduce another method, call the bootstrap, that produces intervals by resampling from the original sample. We show how to perform this method manually, and introduce the syntax in R to produce these intervals virtually via simulations. In Section \@ref(theory_CI) we provide some theoretical foundations to explain the logic behind the bootstrap, compare it with the traditional method, and show advantages and disadvantages of using both methods. Finally, in Section \@ref(summary-CI) we summarize and present extensions of these method.
...
## Tying Sampling Distribution to Estimation {#tracitional-CI}
...
One last clarification. We are often loose when referring to a sample statistic that is use to estimate a population parameter, we call it both *estimator* and *estimate*. To be proper, the **estimator** is the procedure, equation, or method that is used to estimate a parameter from a sample, for example the equation of the sample mean. An **estimate** is the realization of this method when applied to the sample obtained. If an **estimator**, such as the sample mean, has been obtained and calculated, then we say $\bar x = 3.68$ is an **estimate** of the population mean $\mu$. 
...
Since the total number of almonds in the population is 5000, the population mean is given by $$\mu = \sum_{i=1}^{5000}\frac{x_i}{5000}=3.64.$$ and the population standard deviation is given by $$\sigma = \sum_{i=1}^{5000} \frac{(x_i - \mu)^2}{5000}=0.392.$$

But now we would like to estimate the population mean $\mu$ in the case that we do not have access to the entire bowl, only to a single random sample of 100 almonds. We use the sample of 100 almonds `almonds_sample_100` stored in package `moderndive`:
...
The `ID` variable shows the almond chosen from the bowl and its corresponding weight. We can use this sample to obtain the sample statistics:
...
almonds_sample_100 |> 
  summarize(mean_weight = mean(weight), 
            ...
            sample_size = n())
```
In the activity performed in Chapter \@ref(sampling) we obtained many random samples, calculated their sample means, constructed a histogram using these sample means, and show how the histogram is a good approximation of the sampling distribution of the sample mean. We redraw Figure \@ref(fig:sample-mean-100-with-normal) here as Figure \@ref(fig:sample-mean-100-with-normal-redraw).
...
The histogram obtained in Figure \@ref(fig:sample-mean-100-with-normal-redraw) is constructed with sample means almond weights from random samples of size $n=100$. The added red smooth curve is the density curve for the normal distribution with the appropriate expected value and standard error obtained from the sample distribution. The red dot represents the population mean $\mu$ which is the unknown parameter we are trying to estimate. The blue dot is the sample mean $\bar x = 3.68$ from our random sample `almonds_sample_100`.
...
### Interpreting confidence intervals OLD {#one-prop-ci}
...
- Construct the bootsrap samples of size 50
