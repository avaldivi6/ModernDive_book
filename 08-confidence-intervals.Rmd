...
In Chapter \@ref(sampling), sampling was introduced. 
The goal was to estimate the proportion of red balls in the bowl shown in Figure \@ref(fig:sampling-exercise-1). 
...
In Subsection \@ref(sampling-simulation), the sampling activity was expanded "virtually" by performing computer simulations; that is, obtaining thousands of random samples and the corresponding sample proportions of red balls. 
Due to sampling variation, the sample proportions were different for different random samples. 
These sample proportions were used to construct a histogram and visualize the *sampling distribution* of the *sample proportion*. One example could be seen in Figure \@ref(fig:samplingdistribution-virtual-1000).
The histogram was bell-shaped and centered, precisely, at the population proportion, the proportion of red balls in the entire bowl. 
Simulations were also obtained for different sample sizes, as those shown in Figure \@ref(fig:comparing-sampling-distributions-31) where three histograms of sample proportion were drawn, representing the distribution when using three different sample sizes. This comparison showed that as the sample size increased, the sampling variation in the sample proportions decreased.
These results were formalized in Section \@ref(central-limit-theorem) and can be summarized in three points. 
First, the population proportion, the proportion of red balls in the entire bowl, was located precisely at the center of the sampling distribution of the sample proportion. 
Second, the sampling variation, determined by the standard deviation of sample proportions, or *standard error*, is obtained precisely by the formula $SE = \sqrt{p(1-p)/n}$ where $p$ is the population proportion and $n$ is the sample size.
Third, it was shown that, as long as the sample size used is large enough, the sampling distribution of the sample proportion will follow, approximately, a normal curve. The final result is supported by one of the most important theorems in Statistics, the Central Limit Theorem. 
...
However, as described in Section \@ref(sampling-simulation), the sampling activities were useful to study the sampling distribution of the sample proportion. 
In a real-life situation, a *single* random sample is obtained and the sample proportion obtained from this sample is used as the *estimate* of the population proportion. 
This was the situation presented in Section \@ref(sampling-case-study) when the  ["Gallup poll."](https://news.gallup.com/poll/471953/not-expect-return-pre-pandemic-normalcy.aspx) was presented. 
...
## Chocolate-covered almonds {#ci-activity}

In Chapter \@ref(sampling), a bowl with red and white balls was used to study the sampling distribution of the sample proportion. We do here something similar with a bowl of chocolate-covered almonds, as seen in Figure \@ref(fig:bowl_almond). 

```{r bowl_almond, echo=FALSE, fig.cap="A bowl of chocolate-covered almonds.", purl=FALSE, out.width = "40%", purl=FALSE}
include_graphics("images/sampling/almonds/almond_bowl_aux.png")
```

For simplicity we refer to each chocolate covered almond simply as an almond. So, assume we are interested in the average weight in grams of *all*  the almonds in the bowl. We call this the *population average* weight or *population mean* weight.


### One sample {#resampling-tactile}
...
Since the total weight is 88.6 grams, as shown in the Figure \@ref(fig:twenty_five_almonds), the average weight will be $88.6/25 = 3.544$. Is this sample mean a good *estimate* of the population mean? The `moderndive` \index{moderndive!almonds\_sample} package contains the information of this sample in the `almonds_sample` data frame:

```{r}
almonds_sample
```

The `almonds_sample` data frame has `r num_almonds` rows corresponding to each almond with two variables. 
The first variable `ID` identifies individual almonds from top-left to bottom right, by rows in Figure \@ref(fig:twenty_five_almonds), whereas the second variable `weight` corresponds to the corresponding weight of each almond in grams as a numeric variable, also known as a double (`dbl`).

Based on these `r num_almonds` sampled almonds, what can we say about the almonds in the bowl? 
Let's study some properties of our sample by performing an exploratory data analysis. 
Let's first visualize the distribution of the weights of these `r num_almonds_sample` almonds using our data visualization tools from Chapter \@ref(viz). 
...
Observe a slightly left-skewed \index{skew} distribution, since most almonds in the sample weight between 3.6 and 3.8 gr. What is the average weight for the `r num_almonds` sampled almonds? Eyeballing the histogram it appears to be around 3.5. Let's now compute this value exactly using our data wrangling tools from Chapter \@ref(wrangling).
...
Thus, if we're willing to assume that `almonds_sample` is a representative sample from *all* the almonds in the ball, a "good guess" of the average almond weight would be `r x_bar %>% pull(mean_weight) %>% round(2)` gr. This should all start sounding similar to what we did previously in Chapter \@ref(sampling)!

In Chapter \@ref(sampling), the *population* of interest was the bowl of red and white balls and the *population parameter* was the *population proportion* of red balls, denoted by $p$. In order to estimate $p$, a sample of `r n_balls_sample` balls was obtained with a shovel and the proportion of red balls in the sample was computed. This *sample proportion*, $\widehat{p}$, was the *point estimate* for the population proportion.

Here, the population of interest are all the chocolate-covered almonds in the bowl. The population parameter of interest is now the *population mean* weight of all these almonds, a value denoted by the Greek letter $\mu$ (pronounced "mu"). In order to estimate $\mu$, a sample of `r num_almonds_sample` almonds from the bowl was obtained and the *sample mean* weight computed. This sample mean, denoted by $\overline{x}$ (pronounced "x-bar"), is the point estimate of $\mu$. Occasionally, the point estimate for the sample mean is also written $\hat \mu$.

We summarize the relationship and notation between population parameters and correspondin point estimates for both, proportions and means, in Table \@ref(tab:table-ch8-b).
...
### Virtual samples {#virtual-samples-mean}


In the previous Section \@ref(resampling-tactile), we obtained a physical sample from a bowl with chocolate-covered almonds by hand. A we did in Section \@ref(sampling-simulation), we can perform sampling virtually. The bowl seen in Figure \@ref(fig:bowl_almond) is represented by the data frame `bowl_almonds` included in the `moderndive` package. The rows of `bowl_almonds` correspond exactly to the contents of the actual bowl. 

```{r}
bowl_almonds
```

The data frame `bowl_almonds` has `r num_almonds` rows, each representing an almond in the bowl. The first variable `unit_ID` is used as an *identification variable* and the second variable `weight` indicates the weight, in grams, of each chocolate-covered almond. As we did in Section \@ref(sampling-simulation) we use again the `rep_slice_sample()` function to obtain 1000 random samples with a sample `size` set to be 25, and with the number of replicates `reps` set to `1000`. Be sure to scroll through the contents of `virtual_samples` in RStudio's viewer. 

```{r}
virtual_samples <- bowl_almonds |> 
...
Furthermore, the histogram shows a symmetric, bell-shaped distribution that can be approximated well by a normal distribution. While we are dealing with sample means now, the conclusions obtained are strikingly similar to those presented in Chapter \@ref(sampling) when discussing the sampling distribution for the sample proportion.

### The sampling distribution of the sample mean  

As we saw in Chapter \@ref(sampling) when working with sample proportion, the size of the sample has a direct effect on the magnitude of sampling variation of the sample means. To see this, 
...
virtual_mean_weight_25 <- bowl_almonds |> 
  rep_slice_sample(n = 25, reps = 1000)|>
...
virtual_mean_weight_50 <- bowl_almonds |> 
...
virtual_mean_weight_100 <- bowl_almonds |> 
...
if (!file.exists("rds/virtual_sample_means_25.rds")) {
  virtual_samples_25 <- bowl_almonds |>
    rep_slice_sample(n = 25, reps = 1000)
...
if (!file.exists("rds/virtual_sample_means_50.rds")) {
  virtual_sample_means_50 <- bowl_almonds |>
    rep_slice_sample(n = 50, reps = 1000)
...
if (!file.exists("rds/virtual_sample_means_100.rds")) {
  virtual_sample_means_100 <- bowl_almonds |>
    rep_slice_sample(n = 100, reps = 1000)
...
Observe that all three histograms are bell-shaped and appear to center around a middle value,  somewhere around 3.65. In addition, as the sample size increases, the variation among the 1000 sample proportions decreases and there are fewer differences due to sampling variation.
...
Again, this result is really close to the theoretical value of the standard error. The Central Limit Theorem introduced earlier hold here as well. 
...
First, let's perform the virtual analog of resampling once. Recall that the `almonds_sample` data frame included in the `moderndive` package contains the weights of our original sample of `r num_almonds` almonds from the bank. Furthermore, recall in Chapter \@ref(sampling) on sampling that we used the `rep_sample_n()` function as a virtual shovel to sample balls from our virtual bowl of `r nrow(bowl)` balls as follows: 
...
Let's modify this code to perform the resampling with replacement of the `r num_almonds` slips of paper representing our original sample `r num_almonds` almonds:

```{r}
virtual_resample <- almonds_sample %>% 
  rep_sample_n(size = 50, replace = TRUE)
```

Observe how we explicitly set the `replace` argument to `TRUE` in order to tell `rep_sample_n()` that we would like to sample almonds \index{sampling!with replacement} *with* replacement. Had we not set `replace = TRUE`, the function would've assumed the default value of `FALSE` and hence done resampling *without* replacement. Additionally, since we didn't specify the number of replicates via the `reps` argument, the function assumes the default of one replicate `reps = 1`. Lastly, observe also that the `size` argument is set to match the original sample size of `r num_almonds` almonds. 

Let's look at only the first 10 out of `r num_almonds` rows of `virtual_resample`:

```{r}
virtual_resample
```

The `replicate` variable only takes on the value of 1 corresponding to us only having `reps = 1`, the `ID` variable indicates which of the `r num_almonds` almonds from `almonds_sample` was resampled, and `weight` denotes the weight of minting. Let's now compute the mean `weight` in our virtual resample of size `r num_almonds` using data wrangling functions included in the `dplyr` package:

```{r}
virtual_resample %>% 
  summarize(resample_mean = mean(weight))
```

As we saw when we did our tactile resampling exercise, the resulting mean weight is different than the mean weight of our `r num_almonds` originally sampled almonds of `r x_bar %>% pull(mean_weight) %>% round(2)`.
...
Let's now perform the virtual analog of our `r n_resample_friends` friends' resampling. Using these results, we'll be able to study the variability in the sample means from `r n_resample_friends` resamples of size `r num_almonds`. Let's first add a ``reps = `r n_resample_friends` `` argument to `rep_sample_n()` \index{infer!rep\_sample\_n()} to indicate we would like `r n_resample_friends` replicates. Thus, we want to repeat the resampling with the replacement of `r num_almonds` almonds `r n_resample_friends` times.

```{r}
virtual_resamples <- almonds_sample %>% 
  rep_sample_n(size = 50, replace = TRUE, reps = 35)
virtual_resamples
```

The resulting `virtual_resamples` data frame has `r n_resample_friends` $\cdot$ `r num_almonds` = `r n_resample_friends * num_almonds` rows corresponding to `r n_resample_friends` resamples of `r num_almonds` almonds. Let's now compute the resulting `r n_resample_friends` sample means using the same `dplyr` code as we did in the previous section, but this time adding a `group_by(replicate)`:
...
Observe that `virtual_resampled_means` has `r n_resample_friends` rows, corresponding to the `r n_resample_friends` resampled means. Furthermore, observe that the values of `mean_weight` vary. Let's visualize this variation using a histogram in Figure \@ref(fig:tactile-resampling-7).
...
ggplot(virtual_resampled_means, aes(x = mean_weight)) +
  geom_histogram(binwidth = .01, color = "white", boundary = 3) +
  labs(x = "Resample mean weight")
```
...
```{r}
virtual_resampled_means <- almonds_sample %>% 
  rep_sample_n(size = 50, replace = TRUE, reps = 1000) %>% 
  group_by(replicate) %>% 
  summarize(mean_weight = mean(weight))
virtual_resampled_means
