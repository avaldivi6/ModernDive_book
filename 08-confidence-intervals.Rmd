...
In Chapter \ref(chapter7) we worked with sampling. Specifically, we work with some statistics such as the sample proportion, obtained many random samples of the same size from a population of interest, calculate the corresponding sample proportions from each of those samples, and studied what was the distribution of those sample proportions. We learned that

- the sampling distribution of the sampling proportion followed, approximately, the normal distribution; 
- the expected value of the sample proportion, located at the center of the distribution, was exactly equal to the population proportion, and
- the sampling variation of the sample proportion, measured by the standard error of the sample proportion, was equal to the standard deviation of the population divided by the square root of the sample size used to collect the samples.

We also learned that the characteristics of the sampling distribution were not limited to sample proportions. The same characteristics were present when working with the sampling distribution of the sample mean as we showed with the almonds' activities, and they also apply to sampling distributions for the difference of samples means, difference of sample proportions, etc. And for all these cases, the sampling distribution is not dependent on the distribution of the population values. As long as the samples taken are fairly large and we use the appropriate standard error, see \@ref(table-title), we can generalize these results appropriately.

Furthermore, you may remember that the original motivation was to be able to determine population parameter values, such as the proportion of red balls in the entire bowl, or the average weight of almonds in the entire bowl of almonds.

How can we determine the population proportion of red balls in the bowl if we do not have access to the entire bowl? Well, since we have learned the characteristics of the sampling distribution, if we could have access to a large number of random samples, we know that the average of all those sample proportions will be, almost exactly, the population proportion.

However, in real life situations, we do not have access to many random samples. Typically, we have access to a single random sample. How can we determine the population proportion of red balls in the bowl if all we have is a single random sample of red and white balls from the bowl?

The short answer to this question is that we cannot determine exactly what the population proportion is, unless you have access to the entire population or to all the possible random samples of a given size from this population. However, we could __estimate__ the population proportion based on our knowledge of a single sample, and this estimate could be really good. 

But because of sampling variation we know 

In Section \@ref(traditional_CI) we show how to use the sample proportion observed and our knowledge of the sampling distribution of the sample proportion to construct an interval estimate of the population proportion. In Section \@ref(bootstrap_CI) we introduce a technique call the bootstrap that allows us to construct confidence intervals based on resampling from the original sample. In Section \@ref(compare_CI) we compare these two techniques and show advantage and disadvantage of using them. Finally, in Section \@ref(infer) we introduce predefined syntax in R to perform bootstrap confidence intervals.
