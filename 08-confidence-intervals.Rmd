...
<<<<<<< HEAD
In Chapter \@ref(sampling) we introduced *sampling*. In particular, we introduced the *sampling distribution of the sample proportion*. 
...
- The *expected value* of the sample proportion is equal to the *population proportion*. This is statistical jargon that can be understood as follows: when a large number of random samples and corresponding sample proportions are obtained, we *expect* the average of these sample proportions to be equal to the proportion of red balls in the entire bowl, or *population proportions*. This is precisely the readson why the histogram is centered precisely at the value of the *population proportion*.
- The sampling variation, determined by the standard deviation of sample proportions, or *standard error*, is obtained precisely by the formula $SE = \sqrt{p(1-p)/n}$ where $p$ is the population proportion and $n$ is the sample size. By looking at the formula we can see that the standard error is inversely proportional to the square root of the sample size used to obtain the random samples. The formula supports our visual conclusions when comparing histograms for different sample sizes, such as those shown in Figure \@ref(fig:comparing-sampling-distributions-31): the larger the sample size used to obtained random samples, the closer the sample proportions will be to the population proportions. Sampling variation will always be present, but the sample proportions will get closer and closer, on average, to the population proportion.
...
Our original goal was to *estimate* the proportion of red balls in the bowl. Why did we learn about the sampling distribution of the sample proportion? Well, in real-life situations, it is impractical or even impossible to obtain thousands of samples from a population. Instead, as shown in Section \ref(sampling-case-study), we obtain a single sample and associated sample proportion and use it to *estimate* the population proportion. This is the topic of this chapter.
...
knitr::include_graphics("images/sampling/almonds/one_almond.png")
```
...
knitr::include_graphics("images/sampling/almonds/twenty_five_almonds.png")
```
